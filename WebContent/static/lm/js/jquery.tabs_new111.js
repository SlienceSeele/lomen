...
;(function($){
	$.fn.Tabs = function(options){
		return this.each(function(){
			...
			var self = $(this),
				tabBox = self.children( 'div.tab_box111' ).children( 'div' ),
				menu = self.children( 'ul.tab_menu111' ),
				items = menu.find( 'li' ),
				timer;
					
			var tabHandle = function( elem ){
					elem.siblings( 'li' )
						.removeClass( 'current111' )
						.end()
						.addClass( 'current111' );
				...
				autoRun = function( isPrev ){
					var current = menu.find( 'li.current111' ),
					...
					current.removeClass( 'current111' );
					item.addClass( 'current111' );
